...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  github: defineProfile({
    ...
    limitations: ["Detailed usage requires GitHub enhanced billing. Enterprise completeness requires the all-cost-center summary; the adapter never falls back to the cost-center-specific detailed endpoint when that summary is unavailable. Most license budgets cannot stop further usage, so the adapter never calls them enforced solely because prevent_further_usage is true; GitHub Advanced Security is the documented exception. Budget inventory is authoritative only after stable total_count pagination exactly matches the completed collection. Missing account access, unsupported enhanced billing, and unavailable optional Copilot/budget surfaces remain explicit capability states rather than `$0`."],
