...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  oracle: defineProfile({
    ...
    credentialInputs: ["Tenancy OCID, user OCID, API-key fingerprint, tenancy home region, and PEM API signing private key encrypted in server-only provider secret config. COST queries require OCI USAGE_REPORT_READ or USAGE_REPORT_MANAGE; USAGE_ANALYSIS_READ is usage-only.", "Optional verified budget currency, quota compartment OCID, and an explicit bounded list of OCI service names."],
    billing: { visibility: "actual", summary: "USD Usage API COST is direct for the complete reported portion of the UTC month. Non-USD amounts are not FX-converted or counted as USD, and OCI billing-report latency remains explicit." },
    ...
    limitations: ["Usage API DAILY aggregation reports complete day-aligned data and can lag OCI billing publication.", "Optional service detail or limit metadata is fail-open and never prunes prior detail when its source is unavailable, malformed, or over the bounded cardinality limit."],
