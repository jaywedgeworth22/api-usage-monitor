...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  twelvedata: defineProfile({
    ...
    limitations: ["Credit-window semantics are provider-defined and may not be monthly.", "The /api_usage request itself consumes one API credit, so new connections default to a daily sync cadence."],
  ...
  resend: defineProfile({
    ...
    summary: "Validates authentication through the non-sending API-key control plane and captures provider rate-limit headers.",
    reads: ["Count of API keys visible to the supplied credential and API request limit/remaining/reset headers."],
    ...
    limitations: ["API request-rate headers are not the email plan's monthly send quota; authentication success does not prove a specific plan."],
