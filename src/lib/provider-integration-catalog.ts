...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  github: defineProfile({
    ...
    stores: ["Canonical MTD net cost plus product/SKU breakdowns, read-only budget-cap records, and separate Copilot detail components; no repository names, code, issues, alert recipients, or user activity."],
    credentialInputs: ["Organization: GitHub token with Organization Administration read and an org-admin or billing-manager identity.", "Personal user: token with Plan read.", "Enterprise: the documented enterprise billing credential (GitHub may require a classic PAT) and, for GHE.com, the exact `https://api.<enterprise>.ghe.com` API origin.", "Configured account login or slug; legacy org config remains supported."],
    billing: { visibility: "actual", summary: "Enhanced-billing summary netAmount is direct actual MTD spend. Copilot AI-credit/premium detail is stored as a breakdown only, never added to canonical spend a second time. Budget caps are direct but GitHub does not expose their general consumed amount in the documented list response." },
    ...
    limitations: ["Detailed usage requires GitHub enhanced billing. Missing account access, unsupported enhanced billing, and unavailable optional Copilot/budget surfaces remain explicit capability states rather than `$0`."],
