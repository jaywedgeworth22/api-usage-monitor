...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  github: defineProfile({
    ...
    stores: ["Canonical MTD net cost plus product/SKU breakdowns, read-only budget-cap records, and separate Copilot detail components; documented metered budget products are USD while unknown or license-like budget SKUs retain provider-defined units. No repository names, code, issues, alert recipients, or user activity are stored."],
    ...
    billing: { visibility: "actual", summary: "Enhanced-billing summary netAmount is direct actual MTD spend. Copilot AI-credit/premium detail is stored as a breakdown only, never added to canonical spend a second time. Budget caps are direct, but USD is assigned only to documented metered SKUs; unknown or license-like SKUs retain provider-defined units, and GitHub does not expose their general consumed amount in the documented list response." },
    ...
    limitations: ["Detailed usage requires GitHub enhanced billing. Enterprise completeness requires the all-cost-center summary; the adapter never falls back to the cost-center-specific detailed endpoint when that summary is unavailable. Missing account access, unsupported enhanced billing, and unavailable optional Copilot/budget surfaces remain explicit capability states rather than `$0`."],
