...
const DEFINITIONS = [
  ...
  { name: "xai", displayName: "xAI (Grok)", type: "builtin", category: "LLM/AI", helpNote: "Requires xAI Management API key (not an inference key) and Team ID. Reads prepaid balance, postpaid invoice preview, billing cycle, and spending limits.", needsConfig: { fields: [{ key: "teamId", label: "Team ID", placeholder: "xAI team ID", required: true }, { key: "managementKey", label: "Management API key override (optional)", placeholder: "Leave blank to use the API Key above", type: "password", advanced: true }] } },
