...
Authoritative paginated feeds reconcile only after every page has been validated. Stripe and Anthropic reject missing/repeated cursors and malformed successful responses; Cloudflare requires consistent `result_info` totals across the complete subscription list. A partial or ambiguous response books no partial cost and cannot delete previously reconciled state.
