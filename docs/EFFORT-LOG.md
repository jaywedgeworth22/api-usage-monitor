...
## Deployed
- **CLAUDE cap-handoff completion: review-batch gap fixes + P0-P3 sweep + residue integration/cleanup (MONET, owner-directed 2026-07-15) — COMPLETED / ALL MERGED / DEPLOYED.** Picked up docs/rollouts/2026-07-15-pickup-codex-cap-handoff-to-monet.md. Landed PRs #300 (`UsageSnapshot @@index([providerId, fetchedAt])`), #301 (auth trio: XFF tuple keying + HKDF session key + constant-time CRON_SECRET), #306 (dashboard 60s background poll + focus refetch), #307 (salvaged CODEX admission-release regression test, credited), #314 (P2: Cloudflare shared-egress owner-lockout fix via CF CIDR trust check), #315 (P3: test hang guard), #316 (P3: manual-refresh click feedback) — every adversarial-review finding P0-P3 fixed per owner directive; all gates green (local full `npm run verify` + hosted verify/gitleaks/CodeQL). Anthropic receipt reconciliation (handoff §2.4) resolved: ALREADY APPLIED to prod (3 events, $63.96 on provider 45b10ffc…, matching the owner-staged input exactly; offline dry-run reproduced it; redundant re-apply deliberately skipped). Claude receipt evidence recovered from residue bundle to /Users/jay/.secrets/claude-subscription-receipts-2026-07-15.json; Anthropic-billed sub chain ($213.60) already modeled to the cent; Apple-billed active Max 20x $268.11/mo (paid through 2026-07-20) NOT modeled — open owner decision. Worktree registry pruned 74 → ~42 with pre-removal re-verification; all removals archived (patches/tarballs/bundles) to /Users/jay/apps/backups/worktree-prune-2026-07-15/; no origin refs touched, no codex/* pushes; ~28 clean-but-unmerged other-seat lanes left for the separate evaluation handoff row. Full rollout note: docs/rollouts/2026-07-15-monet-cap-handoff-completion.md.
...
## In Progress
- **CODEX cap-pickup handoff execution (CLAUDE → MONET, owner-directed 2026-07-15) — MONET portions COMPLETE; fix lanes with sibling MONET session.** Handoff note: docs/rollouts/2026-07-15-pickup-codex-cap-handoff-to-monet.md (committed alongside this row); closeout: docs/rollouts/2026-07-15-monet-codex-cap-pickup-closeout.md. All four capped-CODEX lanes were verified MERGED+LIVE by CLAUDE pre-handoff (PRs #296/#268/#251+#253/#271). Of the remaining queue: UsageSnapshot composite index MERGED PR #300 (main `0ed5436`); auth-hardening trio PR #301 open, checks green; dashboard auto-refresh WIP — all three owned by the first MONET session (split posted to #agent-sync ts 1784141677 to avoid double-work). Second MONET session completed: (a) archive-first residue/worktree cleanup — 12 superseded worktrees removed (2 named residues incl. local branches, after novelty checks; 10 content-merged squash lanes worktree-only, branches kept), 22 left REPORT-ONLY for the eval-sweep audit, hard exclusions untouched; backups at `/Users/jay/apps/backups/worktree-prune-2026-07-15/`; (b) Anthropic receipt `--apply` disposition RESOLVED read-only — prod already reports Anthropic `receiptCashPaidUsd` $63.96 / 3 events, producible only by the merged importer's HMAC-fingerprinted ingest, so the apply already happened; no log records who ran it (traceability gap flagged to owner). No prod data writes by either session. Row closes to Completed when PR #301 + the auto-refresh PR merge.
- **Provider-family compact account drill-down (CODEX UI lane, root-delegated 2026-07-15) — IMPLEMENTED / FOCUSED GREEN / LOCAL COMMIT READY FOR ROOT REVIEW.** Fresh isolated branch `codex/provider-family-collapse` / worktree `/Users/jay/apps/api-usage-monitor-provider-family-collapse` rebased onto current `origin/main` `c6c4c8f`. Every provider family now occupies one summary row by default, so repeated Anthropic/shared-account rows no longer expand into a long card wall; the labeled disclosure retains every exact account, subscription, external-billing, usage, quota, renewal, and alert detail/link. Hidden detail rows use both semantic `hidden` and inline `display:none` so the existing mobile responsive-table rule cannot force them visible. Node 24.16 focused Vitest passed 1 file / 5 tests, scoped ESLint, TypeScript, and diff-check. Production health is live on `c6c4c8f`; `/` correctly redirects to login. Browser-plugin discovery returned no available browser, so no authenticated visual or interaction claim is made. No external-billing adoption, Infisical ST-primary reader, secrets, Render environment, production writes, push, merge, or deploy.
...
- **Cloudflare explicit renewal-window legacy handoff (CODEX root, 2026-07-15) — IN PROGRESS / RESERVED.** Live guarded probe on deployed `9dacab7` returned sanitized `external_billing_ineligible` without mutating the canonical Subscription. Authenticated sanitized provider state proves the authoritative canonical Workers Paid record is fresh, paid, exact `$5.00` USD monthly, but Cloudflare reports `2026-06-16T19:02:36Z` through its midnight renewal boundary `2026-07-16T00:00:00Z`, so the general exact-calendar-period adoption gate correctly excludes it. Scope: add a Cloudflare-only, handoff-only explicit-window candidate path with tight duration/status/source/identity/freshness/currency/cents/cadence guards; keep general automatic adoption strict; prove no broader provider or PagerDuty behavior. Isolated branch `codex/cloudflare-explicit-renewal-handoff`, worktree `/Users/jay/apps/api-usage-monitor-cloudflare-explicit-renewal-handoff`, base `origin/main` `9dacab7`. Probe flag has been cleared; flag-clear Render replacement is in progress. No production subscription mutation occurred.
  Root-delegated P1 remediation is PR #295 OPEN / FULL-GATE GREEN / HOSTILE-REVIEW LAND on code commit `b548008`, rebased atop `origin/main` `c6c4c8f`: the midnight-window fallback is computed only for the exact preserved built-in Cloudflare legacy row during reconciliation and never enters the global candidate map; another/custom provider, a non-legacy managed Cloudflare row, and a second same-shaped explicit authority fail closed. Focused tests passed 64/64; full `npm run verify` passed ESLint, TypeScript, 89 files / 843 tests, migration/Litestream safety, SQLite backup/startup checks, and production build. Root independently inspected the scope fix and regressions. Branch is pushed and hosted checks are running; no merge, deploy, production, env, secret, provider, database, or PagerDuty mutation.
