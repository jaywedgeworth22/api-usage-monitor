...
## In Progress
- **Congress.Trade full usage-ingest + OTLP activation closeout (CODEX root, owner-directed 2026-07-15) — IN PROGRESS / COORDINATING.** Scope: integrate the existing scheduler-admission/current-main work safely, eliminate the final terminal telemetry loss window for pushed third-party events, verify/complete Congress.Trade event coverage in API Usage Monitor, and activate scheduler then OTLP only after fresh gates and bounded production observation. Existing board rows show OTLP and scheduler are intentionally disabled after the retry-storm/OOM incident; CODEX is auditing current `origin/main` and conflicts before edits or flag changes. No Render config mutation, deploy, provider call, database write, or secret output yet.
