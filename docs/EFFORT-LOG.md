...
## In Progress
- **Render emergency boot-scheduler gate (CODEX operator, owner-directed 2026-07-14) — IN PROGRESS / LOCAL GATE GREEN.** Isolated branch `codex-render-scheduler-emergency-gate` in `/Users/jay/apps/api-usage-monitor-scheduler-gate`, based on `origin/main` `27cf61a`. Production evidence after the no-readiness-probe bridge showed the boot provider scheduler still in progress after five minutes, Prisma P1008 every approximately five seconds, and an authenticated zero-cost ingest smoke returning HTTP 500. This non-overlapping bridge adds a default-on `USAGE_SCHEDULER_ENABLED` switch so the operator can temporarily stop boot polling, isolate SQLite/Litestream recovery, and prove receiver ingest independently. Scope excludes the separately claimed Prisma/Litestream migration files and all provider-money logic. Node 24 full `npm run verify` is green (74 files / 452 tests plus lint, TypeScript, migration safety, backup, startup configuration, and build); independent review, hosted gates, exact-revision deploy, health plus ingest/replay proof, and explicit re-enable criteria remain required.

- **Render `/api/ready` liveness failsafe (CODEX, emergency owner-directed 2026-07-14) — PR #181 MERGED / NO-PROBE FOLLOW-UP READY TO LAND.** PR #181 merged as `938af7f`; deploys `dep-d9b044p9rddc738ipokg` and path-sync redeploy `dep-d9b091ok1i2s73amrn00` are live, and service metadata reports `healthCheckPath=/api/health`. Nevertheless, P1008 `SELECT 1` probes continue every five seconds; only `/api/ready` issues that query. Current-main branch `codex-render-ready-no-db-probe` skips the blocking DB probe only under the temporary compatibility flag while preserving #181's always-200 transport, strict `ok:false/status:not_ready` body, `X-Readiness-Status`, and scheduler/backup/startup diagnostics. Node 24 complete gate passed (73 files / 448 tests plus lint, TypeScript, migration safety, backup, startup config, build, and diff check); independent exact-diff review returned LAND with no P0/P1/P2. Hosted gates, merge/deploy, sustained no-P1008 proof, DB-backed endpoint recovery, retention restoration, and flag removal remain pending.
- **Render `/api/ready` liveness compatibility hotfix (CODEX, emergency owner-directed 2026-07-14) — PR #180 MERGED / SUPERSEDED.** PR #180 merged as `40afc027` after green full/hosted gates and independent review, but its first exact-revision `/api/ready` request timed out beyond 10 seconds. A local pre-#181 skip-probe branch passed 73 files / 446 tests but was never pushed because #181 overlapped the same route. The reconciled current-main follow-up above is the only active implementation.
- **Render SQLite readiness/restart-loop repair (CODEX, PR #178 merged as `d03b1b8`, branch `codex-render-sqlite-readiness`, 2026-07-14) — MERGED / FOLLOW-UP REQUIRED.** Render's paid Starter service is `not_suspended`; this was not a plan/usage-limit failure. Root cause at deployed `cdee64b4`: boot-time retention ran a full exclusive `VACUUM` against the ~129.7 MB SQLite file, strict `/api/ready` probes piled up/timed out (`Prisma P1008`), and Render killed/restarted the sole instance into the same maintenance cycle, producing intermittent `x-render-routing: dynamic-paid-error` 502s. The repair makes scheduled VACUUM explicit-opt-in, uses database-independent `/api/health` in declared Render config, coalesces uncancellable SQLite readiness probes, and adds a bounded database-only cold-start grace that never re-arms after first success. Independent hostile review found no P0/P1/P2; Node 24 full `npm run verify` passed (73 files / 443 tests plus lint, TypeScript, migration, backup, startup-config, and build), and GitHub CI/CodeQL/gitleaks passed. Live service metadata now reports `/api/health`; the no-probe follow-up and sustained runtime verification remain pending. Emergency retention overrides remain temporary; no plan or billing change occurred.
