...
## Completed
- **Verify Hetzner plan currency against the current official API before displaying it as USD (AG, S) — COMPLETED (MERGED PR #216, 2026-07-15).** Confirmed the Hetzner Cloud API pricing contract always returns catalog prices in Euros (`EUR`). Modified the adapter to populate `amountUsd` with the native price while preserving `currency` as `EUR`. The UI displays the native currency (e.g. `€3.50`) correctly, and because `currency !== "USD"`, the amount is kept out of USD budget calculations under `monthlyEquivalentUsd`. Verified with unit tests.
...
## In Progress
- **Congress.Trade full usage-ingest + OTLP activation closeout (CODEX root, owner-directed 2026-07-15) — REBASED LOCAL GATES GREEN / ACTIVATION PENDING.** Branch `codex/telemetry-closeout` contains the current-main scheduler/retention/write-admission replay: external ingest stays mutually exclusive with internal scheduler/retention writes, internal writes are FIFO and reentrant, provider persistence keeps #214 partial-snapshot/postPersistError semantics, and checkpoint/VACUUM runs under admission. Rebased onto current `origin/main` after PR #216; gates pass: `npm run lint`, `npm run typecheck`, focused admission/provider/retention tests 24/24, full Vitest 83 files / 596 tests, `test:migrate-safe`, `test:sqlite-backup`, `test:startup-config`, production `next build`, and `git diff --check`. PR/hosted gates, merge, exact-live deploy, scheduler enable observation, and OTLP enable observation remain. No Render config mutation, provider call, production DB write, deploy, or secret output yet.
