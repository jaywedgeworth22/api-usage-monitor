...
- **Alert persistence config-generation corrective (CODEX builder, delegated 2026-07-14) — HOSTILE FINDINGS REMEDIATED LOCALLY / ROOT RE-REVIEW PENDING.** Branch `codex/alert-persistence-config-generation` in `/Users/jay/.codex/worktrees/api-usage-monitor-alert-persistence-config-generation`, based on fetched `origin/main` `2cf8ab0`; first local commit `594054f` remains unpublished. The corrective now atomically pairs activation evidence/payload mutation with its parent lease; fences child claims/outcomes, summary, and close through exact config/source/transition/payload/parent/child versions; represents stale-snapshot source time separately from its deterministic deadline; separates raw active state from severity-policy delivery eligibility; repairs timed-out aggregate summaries from durable per-channel success without resend; and floors reopened timestamps against resolution/evidence/clock. Required resolver S2/newer active S3, stale trigger message, stale recurrence, severity raise/lower, summary repair, and delayed reopen regressions pass. Node 24 targeted verification: 8 files / 72 tests; alert+migration 40/40; scoped ESLint, TypeScript, inert-URL Prisma validation, and diff check pass. Fetched main is now `0420eb0` (#209); after hostile re-review the branch must rebase while preserving #209's snapshot-capability behavior, then rerun integrated/full gates. Scheduler and OTLP stay disabled; no push/PR/merge/deploy, Render/config/provider/production mutation, provider call, or secret read occurred.
