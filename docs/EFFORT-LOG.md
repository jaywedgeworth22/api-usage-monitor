...
## In Progress
- **Render `/api/ready` liveness failsafe (CODEX, emergency owner-directed 2026-07-14) — READY PR #181 / REBASED GREEN / REVIEW LAND.** Branch `codex-render-readiness-failsafe`, ready PR https://github.com/jaywedgeworth22/api-usage-monitor/pull/181, isolated worktree `/Users/jay/apps/api-usage-monitor-render-readiness-failsafe`, rebased onto PR #180 merge `40afc02`. `/api/ready` keeps strict JSON diagnostics while always returning HTTP 200, failed SQLite probes are cached for 60 seconds, and hung probes stay coalesced so Render's stale five-second poll cannot restart or hammer the sole SQLite process. Post-rebase route suite 11/11, ESLint, TypeScript, and diff-check pass; independent hostile review returned LAND with no P0-P2 findings. Required PR gates, merge, auto-deploy, and live revision/uptime verification remain. This owner-directed failsafe supersedes the narrower compatibility lane for recovery semantics; no Infisical feature files are in scope.
- **Render `/api/ready` liveness compatibility hotfix (CODEX, emergency owner-directed 2026-07-14) — PR #180 MERGED / DEPLOYING / SUPERSEDED BY PR #181.** Branch `codex-render-health-compat`; PR #180 merged as `40afc027`. The narrower bridge returns HTTP 200 only for database-only failure when `RENDER_READINESS_HTTP_COMPATIBILITY=true`, while retaining `{ ok:false, status:"not_ready" }` and preserving HTTP 503 for scheduler, backup, or startup failures. Node 24 full verify, hosted CI/CodeQL/gitleaks, and independent review passed. The flag was enabled with merge semantics; Render deploy `dep-d9b00ggddkuc73dp2d6g` queued behind the env deploy. Live logs still showed five-second Prisma P1008 probes and a kill after the five-minute grace, so PR #181 above is the broader replacement with liveness-safe transport for every diagnostic state plus a 60-second failed-probe backoff.
